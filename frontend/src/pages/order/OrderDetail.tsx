...
import {
    ...
    updateOrder, updateOrderReturnInfo
} from "../../redux/reducers/orderSlice.ts";
...
interface ReturnStatusTimestamps {
    pending: string,
    initiated: string,
    completed: string,
    refunded: string
...
const formatReturnStatusTimestamps = (returnStatusTimestamps: ReturnStatusTimestamps) => {
    const returnStatusLabels: { [key: string]: string } = {
...
const ClassificationDetail: React.FC<Props> = ({id}) => {
    ...
    const [returnButtonTitle, setReturnButtonTitle] = useState<string | null>('');
    const [currentReturnStatus, setCurrentReturnStatus] = useState<string>('');
    const [newReturnStatus, setNewReturnStatus] = useState<string | null>('');

    const updateReturnInfoStatus = useSelector((state: RootState) => state.orders.updateReturnInfoStatus);
    const updateReturnInfoError = useSelector((state: RootState) => state.orders.updateReturnInfoError);
    ...
    useEffect(() => {
        if (updateReturnInfoStatus === stateStatus.loadingState) setIsReturnLoading(true)
        else setIsReturnLoading(false)
    }, [updateReturnInfoStatus]);
    ...
    useEffect(() => {
        if (order) {
            ...
            if (order.status === 'returned' && order.returnInfo !== null) {
                setCurrentReturnStatus(order.returnInfo.status);
            }
        }
    }, [order]);
    ...
    useEffect(() => {
        switch (currentReturnStatus) {
            case 'pending': {
                setReturnButtonTitle('Chấp thuận hoàn trả');
                setNewReturnStatus('initiated');
                break;
            }
            case 'initiated': {
            ...
            case 'completed': {
                setReturnButtonTitle('Tiến hành hoàn tiền');
                setNewReturnStatus('refunded');
                break;
            }
            default: {
                setReturnButtonTitle(null);
                setNewReturnStatus(null);
                break;
            }
        }
    }, [currentReturnStatus]);
    ...
    const handleCancelReturn = () => {
        const isConfirmed = window.confirm("Chắc chắn hủy yêu cầu hoàn trả hàng?");
        if (isConfirmed) {
            dispatch(updateOrderReturnInfo({id: id, newReturnStatus: 'cancelled'}));
        } else {
            dispatch(resetOrderUpdateStatus())
        }
    }
    ...
    useEffect(() => {
        if (updateReturnInfoStatus === stateStatus.failedState) {
            if (updateReturnInfoError) {
                alert(updateReturnInfoError.error)
            ...
            <div className={'relative mt-3'}>
                ...
                <div className="mb-3 size-full">
                    ...
                    </table>
                    {
                        order && order.returnInfo && (
                            <div className={'w-full border border-gray-300 p-3 mt-3 rounded'}>
                                ...
                                {order.returnInfo.statusTimestamps ? (
                                    <ul className={'list-disc pl-5'}>
                                        {formatReturnStatusTimestamps(order.returnInfo.statusTimestamps).map((status, index) => (
                                            <li key={index} className={'text-gray-600 text-lg'}>
                                                <span className="font-semibold text-black">{status.label}:</span>{" "}
                                                {status.formatStatusTimestamps}
                                            </li>
                                        ))}
                                    </ul>
                                ) : (
                    ...
                    <div className={'w-full flex justify-end mt-6'}>
                        ...
                        {
                            returnButtonTitle && <button onClick={handleUpdateReturnInfo}
                                                         className={`bg-blue-500 mt-1 w-1/4 ms-5 text-white rounded p-2`}
                                                         disabled={isReturnLoading}>
                                {returnButtonTitle}
                            </button>
